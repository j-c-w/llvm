...
SESEFunction::SESEFunction(llvm::Value* begin, llvm::Value* successor) : old_function(nullptr)
{
    ...
    if(begin_block == nullptr || successor_block == nullptr ||
       begin_block->getParent() != successor_block->getParent()) return;

    old_function = begin_block->getParent();
...
llvm::Function* SESEFunction::make_function()
{
    ...
    for(llvm::Argument& arg : new_function->args())
    {
