                                                      ...
                                                      std::vector<std::map<std::string,Value*>> histos)
{
    ...
    argument_types.push_back(reduction_ptr_type);
    ...
    FunctionType* function_type = FunctionType::get(function.getReturnType(), argument_types, false);

    auto new_function = Function::Create(function_type, function.getLinkage());
    new_function->arg_begin();
    function.stealArgumentListFrom(*new_function);
    delete new_function;

    Argument* reduction_argument = function.arg_begin();

    std::vector<Argument*> actual_arguments;

    for(unsigned i = 1; i < arguments.size()+1; i++)
    {
        actual_arguments.push_back(function.arg_begin()+i);
    }
